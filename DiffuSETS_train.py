...
if __name__ == "__main__":
    ...
    path = '/data/0shared/laiyongfan/data_text2ecg/mimic_vae'
    n_channels = 4
    num_train_steps = 1000
    diffused_model = DDPMScheduler(num_train_timesteps=num_train_steps, beta_start=0.00085, beta_end=0.0120)
    
    # dataset = ECGdataset(dataset_path, text_embed_path)
    dataset = VAE_MIMIC_IV_ECG_Dataset(path=path)
    
    net = ECGconditional(num_train_steps, resolution=128, kernel_size=7, num_levels=5, n_channels=n_channels)
    ...
    for i in range(1, epoch_number + 1):
        ...
        if (mean_loss < min_loss):
            min_loss = mean_loss
            torch.save(net.state_dict(), save_weights_path+'ecg2chan_input20K_1000ts_'+str(i)+'.pth')
            print('ecg2chan_input20K_1000ts_'+str(i)+'.pth has been saved.')
        if (i % 50 == 0):
            torch.save(net.state_dict(), save_weights_path+'ecg2chan_1000ts_+epoch('+str(i)+').pth')

        e_t = time.time()
        print("Epoch Time Used: ", e_t - s_t, "s; Total Time Used: ", e_t - start_time, "s")
        sys.stdout.flush()
