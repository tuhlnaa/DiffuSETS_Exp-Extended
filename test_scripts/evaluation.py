...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ptbxl", action="store_true",
        help="test ptbxl result"
    )
    args = parser.parse_args()

    exp_type = args.exp_type
    if 'nonoc' in exp_type: 
        use_latent = False 
        use_condition = False 
    elif 'noc' in exp_type: 
        use_latent = True 
        use_condition = False 
    elif 'novae' in exp_type: 
        use_latent = False 
        use_condition = True 
    else: 
        use_latent = True 
    ...
    # CLIP model
    clip_model_root = './prerequisites/clip_model.pth'
    ...
    # UNET
    n_channels = 4 if use_latent else 12 
    num_train_steps = 1000
    n_inference_steps = 1000 if use_latent else 200 
    ...
    state = generate_feature_matrix(sample_dir=save_path, clip_model=clip_model, device=device, decoder=decoder, use_all_batch=True, use_latent=use_latent)
    ...
    result_line = f'{exp_type}_p\t' if args.ptbxl else f'{exp_type}\t'
