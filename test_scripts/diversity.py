...
def batch_generate_ECG(nums, 
    ...
    for _, (x, y) in enumerate(test_dataloader):
        ...
        latent = x
        text = y['text'][0]
        gender = 1 if y['gender'][0] == 'M' else 0
        ...
        if len(text) > 200:
            text = text[:200]
        save_sample_path = os.path.join(save_path, number_str +  '-' + text + '/')
        ...
        if save_img:
            ...
            ecg_plot.plot(input_.transpose(1, 0), 102.4)            
            ...
            for j in range(batch):
                ...
                ecg_plot.plot(output_.transpose(1, 0), 102.4, show_grid=False)            
...
if __name__ == "__main__":
    ...
    save_path = 'exp/test_diversity_sample'
    device_str = "cuda:0"
