...
attribute_emb = pd.read_csv('/data/0shared/chenjiabo/DiffuSETS/data/attribute_embed.csv')

class SelfAttention(nn.Module):
    def __init__(self, vector_size, dim, hidden_dim=16, num_heads=4, text_embed_dim=1536, key_embed_dim=1536, value_dim=1, value_embed_dim=1):
    ...
    def forward(self, x, text_embed, dict):
        ...
        if text_embed.shape:
            ...
            key_embed = attribute_emb.loc[attribute_emb['attribute'] == key, 'embed'].values[0]
            key_embed = np.array(eval(key_embed))
            key_embed = np.repeat(key_embed[np.newaxis, :], text_embed.shape[1], axis=0)
            key_embed = np.repeat(key_embed[np.newaxis, :, :], text_embed.shape[0], axis=0)
            key_embed = torch.Tensor(key_embed)
            key_embed = key_embed.to(device)
            '''
        ...
        for key, value in dict.items():
            ...
            key_embed = attribute_emb.loc[attribute_emb['attribute'] == key, 'embed'].values[0]
            key_embed = np.array(eval(key_embed))
            key_embed = np.repeat(key_embed[np.newaxis, :], value_embed.shape[1], axis=0)
            key_embed = np.repeat(key_embed[np.newaxis, :, :], value_embed.shape[0], axis=0)
            key_embed = torch.Tensor(key_embed)
            key_embed = key_embed.to(device)
            '''
...
class Block(nn.Module):
    ...
    def forward(self, x, t, text_embed, conditon):
        ...
        shortcut = self.attention(shortcut, text_embed, conditon)
...
class DownsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, dim, number_of_diffusions, 
        ...
        self.down = nn.Conv1d(n_outputs, n_outputs, 3, 2, padding=1)#nn.MaxPool1d(2)
        self.block = Block(n_inputs, n_outputs, dim, number_of_diffusions, kernel_size=kernel_size, n_heads=n_heads, hidden_dim=hidden_dim, text_embed_dim=text_embed_dim)
...
class UpsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, dim, number_of_diffusions,
        ...
        self.block = Block(n_inputs, n_outputs, 2*dim, number_of_diffusions, kernel_size=kernel_size, n_heads=n_heads, hidden_dim=hidden_dim, text_embed_dim=text_embed_dim)
...
class BottleneckNet(nn.Module):
    def __init__(self, resolution, n_channels, number_of_diffusions,
                 kernel_size=3, n_heads=None, hidden_dim=None, text_embed_dim=1536):
        ...
        self.text_embedding_layer = nn.Linear(text_embed_dim, resolution)

        if n_heads is None and hidden_dim is None:
            self.attention = SelfAttention(resolution, n_channels, text_embed_dim=text_embed_dim)
        elif n_heads is None and hidden_dim:
            self.attention = SelfAttention(resolution, n_channels, hidden_dim=hidden_dim, text_embed_dim=text_embed_dim)
        elif n_heads and hidden_dim is None:
            self.attention = SelfAttention(resolution, n_channels, num_heads=n_heads, text_embed_dim=text_embed_dim)
        elif n_heads and hidden_dim:
            self.attention = SelfAttention(resolution, n_channels, num_heads=n_heads, hidden_dim=hidden_dim, text_embed_dim=text_embed_dim)
    ...
    def forward(self, x, t, text_embed, conditon):
        ...
        out = self.attention(out, text_embed, conditon)
...
class ECGconditional(nn.Module):
    def __init__(self, number_of_diffusions, resolution=512, kernel_size=3, num_levels=4, n_channels=12, text_embed_dim=1536):
...
if __name__ == "__main__":
    ...
    dataset_path = "/data/0shared/chenjiabo/ecg_diffusion/diffusion_model_ecg/data/PTBXL.npy"
